...
- The opening description must also make clear that Behavior Rocker is a reusable control that can be customized here, downloaded for use on any website or app, and used under the MIT License.
...
- Keep a code-export section directly below the live sample. It must regenerate from every current customization, offer React, Vue 3, and HTML with JavaScript, and copy a standalone usable component rather than an import from an unpublished package.
